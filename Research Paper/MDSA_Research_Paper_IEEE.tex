...
\begin{abstract}
Enterprise AI systems face significant challenges including high operational costs, excessive latency, and limited control over data privacy. Current general-purpose frameworks like LangChain and AutoGen, while powerful, struggle with the specific requirements of domain-focused applications where specialized knowledge and fast response times are critical. We present MDSA (Multi-Domain Specialized Agentic Orchestration), a novel framework that combines lightweight domain classification using small language models with specialized knowledge bases to deliver superior performance for enterprise deployments. Our approach uses a 67-million parameter TinyBERT router for domain classification, achieving 94.1\% accuracy on 10,000 test queries while maintaining router latency of 40-60ms. Combined with a dual RAG architecture featuring both global and domain-specific knowledge bases, MDSA achieves median end-to-end latency of 348-391ms depending on hardware, with 87.3\% RAG retrieval precision. The framework enables zero-cost local deployment with quantized models, consuming under 1GB memory. Experimental evaluation on a production-scale test suite across 5 domains demonstrates practical viability, with direct compute cost of \$0.0006-0.0009 per request. The framework is released as open source to support enterprise AI deployments requiring performance, cost efficiency and data privacy.
\end{abstract}
...
This paper introduces MDSA, a framework specifically designed for domain-focused enterprise applications where queries can be reliably classified into known categories. Our key contributions are:

\begin{itemize}
\item A hybrid orchestration architecture combining a lightweight TinyBERT router (67M parameters) with domain-specialized models, achieving 94.3\% routing accuracy while maintaining sub-100ms classification latency
\item A dual RAG system featuring both global knowledge bases (10,000+ documents) and domain-specific stores (1,000 documents per domain), achieving 87.3\% Precision@3 for relevant document retrieval
\item A multi-level caching strategy utilizing domain embedding cache and response cache, providing 200x speedup on repeated queries
\item Comprehensive evaluation demonstrating 2.4x improvement in response latency and 60\% reduction in memory consumption compared to LangChain, while maintaining higher accuracy
\item Open-source implementation supporting both local (Ollama) and cloud model deployment, enabling zero-cost operation for privacy-sensitive applications
\end{itemize}
...
LangChain \cite{langchain2023} emerged as one of the first comprehensive frameworks for building LLM applications, providing abstractions for chains, agents, and memory. While powerful for general use cases, LangChain's flexibility comes with performance overhead -- our measurements show average latency of 1,850ms for domain-routed queries, compared to 625ms for MDSA. The framework prioritizes generality over optimization for specific deployment scenarios.

AutoGen \cite{wu2023autogen} focuses on multi-agent conversations and code execution, enabling agents to debate and refine responses. This approach works well for research and exploratory tasks but introduces significant latency (2,100ms average) and memory overhead (3,500MB) that make it less suitable for high-throughput production systems. Additionally, the framework assumes cloud model access, limiting options for local deployment.

CrewAI \cite{crewai2024} provides role-based orchestration where agents have specific responsibilities within workflows. While this matches some enterprise use cases, the framework's focus on sequential task delegation introduces latency (1,950ms average) and doesn't optimize for the common scenario where query classification enables direct routing to domain specialists.
...
The router is trained on domain-specific examples, achieving 94.3\% accuracy on our evaluation set. Inference latency ranges from 25-61ms depending on query length, which is significantly faster than using full-size models (200-500ms) \cite{langchain2023}.

\subsubsection{Domain Embedding Cache}
...
If similarity exceeds threshold $\tau$ (typically 0.95), we use the cached domain classification, reducing latency by 80\% (25-61ms vs 125-310ms without cache). Cache hit rate averages 60-80\% in production FAQ scenarios.

\subsubsection{Dual RAG System}
MDSA implements a two-tiered knowledge architecture:
...
This hybrid approach achieves 87.3\% Precision@3 for relevant documents, compared to 76.8\% for global-only retrieval in our evaluation.

\subsubsection{Response Cache}
For queries with high semantic similarity to previous queries, MDSA implements response caching using MD5 hashing of normalized queries. Cache lookup adds <1ms overhead while providing 200x speedup (<10ms vs 625ms) on cache hits. The cache uses FIFO eviction with configurable size limits.
...
\item Router computes embedding $e(q)$ and checks domain embedding cache
\item If cache hit (similarity $>$ $\tau$), use cached domain; else classify with TinyBERT
\item Check response cache using normalized query hash
\item If cache miss, retrieve context from dual RAG system
\item Route to domain specialist or reasoning model based on complexity
\item Model generates response $r$
\item Cache response and return to user
\item Log metrics to monitoring dashboard
\end{enumerate}

This architecture achieves average end-to-end latency of 625ms, with cache hits reducing this to <10ms.
...
We evaluate MDSA against three established frameworks: LangChain (v0.0.350), AutoGen (v0.2.11), and CrewAI (v0.28.0). All experiments run on a consistent hardware platform:

\begin{itemize}
\item CPU: Intel Core i7-9700K (8 cores, 3.6 GHz)
\item RAM: 16GB DDR4
...
For local model deployment, we use Ollama with Deepseek-v3.1 (6.8B parameters) as the domain specialist model. Cloud-based experiments use OpenAI GPT-3.5-turbo for cost comparison fairness.

Evaluation data consists of 1,000 queries across five domains (200 queries per domain), hand-labeled by domain experts. Queries range from simple factual lookups to complex multi-step reasoning tasks, representative of real enterprise workloads.
